...
class DigitalOutputSequence(object):
    ...
    def add_step(self, step_id, dout_vec):
        ...
        if (step_id in self.dout_list):
            raise ValueError("The step id (" + str(step_id) + ") is already registered.")

        if (len(self.dout_list) == self._MAX_DOUT_STEPS):
            raise ValueError("No more digital output vectors can be added. (max=" + str(self._MAX_DOUT_STEPS) + ")")
        
        if (not isinstance(dout_vec, DigitalOutputVector)):
            raise ValueError("invalid digital output vector " + str(dout_vec))

        self.dout_list[step_id] = dout_vec
        return self


    def num_steps(self):
        return len(self.dout_list)
    ...
    def serialize(self):
        ...
        dout_list = sorted(self.dout_list.items())
