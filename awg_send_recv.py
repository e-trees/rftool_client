...
DAC_SAMPLES = 1024 * 1024
ADC_SAMPLES = 1024 * 1024
DATA_DEVIDES = 2 * 1024
PLOT_DIVIDES = 2 * 1024
CROP_PLOT = [0, 512]
FFT_SIZE = 32768
...
def set_wave_sequence(awg_sa_cmd):
    ...
    # AWG に波形シーケンスをセットする
    awg_sa_cmd.set_wave_sequence(awg_id = awgsa.AwgId.AWG_0, wave_sequence = wave_sequence_0)
...
def set_capture_sequence(awg_sa_cmd, seq_0, seq_1):
    ...
    # キャプチャシーケンスの定義
    capture_sequence_0 = (awgsa.CaptureSequence(ADC_FREQ, enable_iq_mixer = False)
        .add_step(step_id = 0, capture = capture_0)
        .add_step(step_id = 1, capture = capture_1))

    capture_sequence_1 = (awgsa.CaptureSequence(ADC_FREQ, enable_iq_mixer = False)
